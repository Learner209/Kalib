...
class _ColorfulFormatter(logging.Formatter):
    ...
    def formatMessage(self, record):
        ...
        if record.levelno == logging.DEBUG:
            prefix = colored("DEBUG", "blue", attrs=["bold"])
        elif record.levelno == logging.INFO:
            prefix = colored("INFO", "green", attrs=["bold"])
        elif record.levelno == logging.WARNING:
            prefix = colored("WARNING", "yellow", attrs=["bold", "blink"])
        elif record.levelno == logging.ERROR:
            prefix = colored("ERROR", "red", attrs=["bold", "underline"])
        elif record.levelno == logging.CRITICAL:
            prefix = colored("CRITICAL", "magenta", attrs=["bold", "blink", "underline"])
        else:
        ...
        if distributed_rank == 0:
            ...
            if color:
                formatter = _ColorfulFormatter(
                    ...
                    colored("[%(asctime)s.%(msecs)03d]: ", "green") + "%(pathname)s:%(lineno)d: " + "%(message)s",
                    datefmt="%m/%d %H:%M:%S",
                    root_name=name,
